...
PT Manager must handle errors gracefully and inform users appropriately. Estes códigos aplicam-se
sobretudo à Checkout Session (criação) e à Subscription (cobrança de renovação) — não a
charges/refunds avulsos, que não existem no modelo atual.
...
A verificação de assinatura fica na Infrastructure (a implementar), que normaliza o evento antes
de chamar `ProcessPaymentWebhookHandler` (já existe). Ver `references/webhook-events.md` para os
eventos reais (`checkout.session.completed`, `customer.subscription.updated`, etc.).
...
public async Task ProcessWebhookAsync(string json, string signature)
{
    try
    {
        ...
        var normalized = NormalizePaymentEvent(stripeEvent); // mapear para PaymentEventKind
        await _processPaymentWebhookHandler.HandleAsync(normalized, ct);
    }
    catch (StripeException ex)
    {
