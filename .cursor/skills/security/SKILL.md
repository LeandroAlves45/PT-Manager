---
name: security
description: Expert in ASP.NET Core API authentication, authorization, rate limiting, and abuse prevention for PT Manager. Secures endpoints against common attacks (injection, brute force, API abuse).
color: red
emoji: 🔐
...
### Aplicação no PT Manager (ASP.NET Core)

O backend do PT Manager é C#/.NET 10, não Node/Express — os princípios acima aplicam-se, mas a implementação concreta usa o stack nativo do ASP.NET Core em vez das bibliotecas TypeScript ilustradas:

- **JWT**: `Microsoft.AspNetCore.Authentication.JwtBearer`, access token de curta duração (15 min) + refresh token opaco com rotação e deteção de reuso (nunca reimplementar JWT à mão como no exemplo `jsonwebtoken` acima)
- **Rate limiting**: `Microsoft.AspNetCore.RateLimiting` (nativo desde .NET 7), política mais restritiva em `/auth/login`, `/auth/signup`, `/auth/refresh`
- **CORS**: lista explícita de origens (Vercel produção + previews), nunca wildcard, configurada em `Program.cs` — ver `00_ARCHITECTURE.md §5.2`
- **Validação de input**: `FluentValidation` nos handlers da Application (chamada explícita, sem `.AspNetCore` pipeline automático), nunca validação manual campo-a-campo como no exemplo `validateInput` acima
- **Multi-tenant**: `owner_trainer_id` nunca vem do body/query — é sempre resolvido de `ITenantContext` a partir do JWT, via EF Core Global Query Filters (`00_ARCHITECTURE.md §6`)

Ver skill `database-migrations` e `.claude/project/00_ARCHITECTURE.md §5-§9` para o desenho completo de auth/multi-tenancy/rate limiting do projeto.
