...
def client(db):
    """
    TestClient com dependência get_session substituída e startup sem migrations/seeds/scheduler.
    """
    ...
    with patch("app.db.init_db.run_migrations"), patch(
        "app.db.migrate.run_migrations"
    ), patch("app.main.seed_superuser"), patch("app.main.seed_pack_types"), patch(
        "app.main.seed_demo_data"
    ), patch(
        "app.workers.scheduler.start_scheduler"
    ), patch(
        "app.workers.scheduler.shutdown_scheduler"
    ):
