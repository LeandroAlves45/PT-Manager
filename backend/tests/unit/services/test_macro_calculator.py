"""
Testes unitários para os cálculos de TMB, TDEE e macros nutricionais.
"""
...
class TestCalculateTMBAllFormulas:
    ...
    def test_devolve_tres_formulas(self, male_athlete):
        ...
        assert (
            len(tmb_results) == 3
        ), "A função deve devolver exatamente 3 resultados, um por fórmula."
    ...
    def test_formulas_ordenadas_por_tdee_crescente(self, male_athlete):
        ...
        # Zip (results, results[1:]) para criar pares consecutivos e verificar a ordem
        # Verifica que cada elemento tem TDEE menor ou igual ao seguinte
    ...
    def test_tmb_e_tdee_sao_positivos(self, male_athlete):
        ...
        for result in results:
            assert (
                result.tmb > 0
            ), f"{result.formula}: TMB deve ser positivo, mas devolveu {result.tmb}"
            assert (
                result.tdee > 0
            ), f"{result.formula}: TDEE deve ser positivo, mas devolveu {result.tdee}"
    ...
    def test_mulher_tem_tmb_diferente_de_homem(self, male_athlete, female_client):
        ...
        # Compara TMB da mesma fórmula (Harris-Benedict) entre homem e mulher
        # para verificar que são diferentes
    ...
    def test_activity_key_invalida_levanta_erro(self, male_athlete):
        ...
        """
        # "super_active_nao_existe" não é uma chave válida
        dados = {**male_athlete, "activity_key": "super_active_nao_existe"}
    ...
    def test_sex_invalido_levanta_erro(self, male_athlete):
        """
        Verifica que sex inválido não é tratado como female por engano.
        """
        dados = {**male_athlete, "sex": "other"}

        with pytest.raises(ValueError) as exc_info:
            calculate_tmb_all_formulas(**dados)

        assert "Sexo inválido" in str(exc_info.value)
    ...
    def test_todos_os_niveis_de_atividade_funcionam(self, activity_key):
        ...
        assert (
            len(results) == 3
        ), f"Falhou para activity_key='{activity_key}': deve devolver 3 resultados"
...
class TestMacrosFromPercentages:
    """Testes para cálculo de macros a partir de percentagens."""
    ...
    def test_kcal_total_consistente_com_gramas(self):
        ...
        kcal_calculado = (
            (result.protein_g * 4) + (result.carbs_g * 4) + (result.fats_g * 9)
        )
    ...
    def test_g_por_kg_calculado(self):
        ...
        assert result.protein_g_per_kg == pytest.approx(
            result.protein_g / 100.0, abs=0.1
        )
    ...
    def test_kcal_zero_levanta_erro(self):
        """
        Verifica que objetivo calórico zero é rejeitado.
        """
        with pytest.raises(ValueError) as exc_info:
            calculate_macros_from_percentages(
                kcal_target=0,
        ...
        assert "Calorias deve ser um valor positivo." in str(exc_info.value)
    ...
    def test_varios_cenarios_retornam_valores_positivos(self, kcal, p, h, g):
        ...
        assert (
            result.protein_g > 0
        ), f"Proteína deve ser positiva para TDEE={kcal}, P={p}%"
        assert (
            result.carbs_g > 0
        ), f"Hidratos devem ser positivos para TDEE={kcal}, H={h}%"
...
class TestMacrosFromGramsPerKg:
    """Testes para cálculo de macros a partir de rácios por kg corporal."""
    ...
    def test_peso_zero_levanta_erro(self):
        """
        Verifica que peso zero é rejeitado no cálculo por g/kg.
        """
        with pytest.raises(ValueError) as exc_info:
            calculate_macros_from_grams_per_kg(
                weight_kg=0,
        ...
        assert "Peso deve ser um valor positivo." in str(exc_info.value)

    def test_racio_zero_levanta_erro(self):
        """
        Verifica que rácios zero são rejeitados no cálculo por g/kg.
        """
        with pytest.raises(ValueError) as exc_info:
            calculate_macros_from_grams_per_kg(
                weight_kg=80.0,
...
class TestGetActivityFactorOptions:
    """Testes para as opções de fatores de atividade."""
    ...
    def test_cada_opcao_tem_campos_obrigatorios(self):
        ...
        for option in options:
            ...
            assert (
                not missing
            ), f"Opção '{option.get('key')}' está faltando campos: {missing}"
    ...
    def test_chaves_correspondem_aos_fatores(self):
        ...
        assert option_keys == set(ACTIVITY_FACTORS.keys()), (
            f"As chaves das opções {option_keys} devem corresponder às chaves "
            f"de ACTIVITY_FACTORS {set(ACTIVITY_FACTORS.keys())}"
        )

    def test_fatores_correspondem_ao_dicionario(self):
        """
        Verifica que cada opção corresponde ao valor em ACTIVITY_FACTORS.
        """
