...
class TestPasswordHashing:
    ...
    def test_hash_starts_with_bcrypt_prefix(self):
        """O hash bcrypt deve começar com o prefixo $2b$."""
        result = hash_password("anotherpassword456")
        assert result.startswith("$2b$")

    def test_hash_is_not_plain_text(self):
        """O hash deve ser diferente da password original."""
        password = "mysecretpassword123"
        result = hash_password(password)
        assert result != password

    def test_same_password_produces_different_hashes(self):
        """A mesma password deve produzir hashes diferentes (salt aleatório)."""
    ...
    def test_verify_password_empty_string(self):
        """Password vazia deve retornar False quando comparada com hash não vazio."""
...
class TestJWTToken:
    ...
    def test_create_access_token_returns_string(self):
        ...
        token = create_access_token(
            subject="user123", role="trainer", full_name="Test User"
        )
        assert isinstance(token, str)
        assert len(token) > 0

    def test_token_contains_correct_subject(self):
        """O payload do token deve conter o user_id correto no campo 'sub'."""
        user_id = "test_user123"
        token = create_access_token(
            subject=user_id, role="trainer", full_name="Test User"
        )
    ...
    def test_token_contains_correct_role(self):
        ...
        token = create_access_token(
            subject="user123", role="client", full_name="Test User"
        )
        payload = jwt.decode(token, settings.secret_key, algorithms=[ALGORITHM])
    ...
    def test_token_contains_client_id_when_provided(self):
        ...
        token = create_access_token(
            subject="user123", role="client", full_name="Test User", client_id=client_id
        )
        payload = jwt.decode(token, settings.secret_key, algorithms=[ALGORITHM])
    ...
    def test_token_without_client_id_has_no_cid_claim(self):
        ...
        token = create_access_token(
            subject="trainer-123", role="trainer", full_name="Test User"
        )
        payload = jwt.decode(token, settings.secret_key, algorithms=[ALGORITHM])
