...
class TestLogin:
    """Testes de login e persistência de tokens activos."""
    ...
    def test_login_persists_token_in_database(
        self, client, trainer_user, db, api_key_headers
    ):
        """
        ...
        assert (
            len(tokens) == 1
        ), "Deve existir exactamente um token activo por utilizador"

    def test_login_second_login_replaces_previous_token(
        self, client, trainer_user, db, api_key_headers
    ):
        """
        ...
        assert (
            len(tokens) == 1
        ), "Dois logins consecutivos devem resultar num unico token activo"

    def test_login_wrong_password_returns_401(
        self, client, trainer_user, api_key_headers
    ):
        """
...
class TestLogout:
    """Testes de logout e invalidação de tokens activos."""
    ...
    def test_logout_remove_token_from_database(
        self, client, trainer_user, db, api_key_headers
    ):
        """
    ...
    def test_after_request_logout_returns_401(
        self, client, trainer_user, db, api_key_headers
    ):
        """
        ...
        assert resp.status_code == 401, (
            "Token deve ser rejeitado apos logout, mesmo que o JWT ainda "
            "não tenha expirado"
        )
