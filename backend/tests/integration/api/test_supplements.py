...
class TestCatalogoSuplementos:
    """Testes do catálogo de suplementos do trainer."""
    ...
    def test_listar_suplementos_proprios(
        self, client, trainer_headers, supplement_in_db
    ):
    ...
    def test_isolate_multi_tenant_list(
        self, client, trainer_headers, second_trainer_headers, supplement_in_db, db
    ):
        """
        ...
        assert (
            "Creatina Monohidratada" not in nomes_b
    ...
    def test_client_cannot_see_trainer_notes(
        ...
        assert resp.status_code == 200
        assert (
            "trainer_notes" not in resp.json()
...
class TestSupplementAssignment:
    """Testes de atribuição de suplementos a clientes."""
...
class TestSupplementsPortal:
    """Testes do portal do cliente para suplementos atribuídos."""

    def test_client_sees_their_supplements(
        self, client, trainer_headers, client_headers, client_record, supplement_in_db
    ):
        """
