...
class TestGenerateInviteLink:
    """Testes da geração de links de convite pelo trainer."""
    ...
    def test_generate_second_invite_invalidates_first(
        ...
        db.refresh(client_user)
        assert client_user.invite_token_hash == _sha256(
            resp2.json()["invite_link"].split("/invite/")[-1]
        )
    ...
    @patch(
        "app.services.invite_service.EmailService.send_invite_email",
        side_effect=RuntimeError("SMTP indisponível"),
    )
    def test_generate_invite_succeeds_when_email_fails(
        self,
        _mock_send_email,
        client,
        trainer_headers,
        ...
        """
        Falha no envio de email não deve bloquear a geração do link (best-effort).
        """
        resp = client.post(
            f"/api/v1/clients/{client_record.id}/generate-invite",
            headers=trainer_headers,
        ...
        assert resp.status_code == 201
        assert "/invite/" in resp.json()["invite_link"]

        db.refresh(client_user)
        raw_token = resp.json()["invite_link"].split("/invite/")[-1]
        assert client_user.invite_token_hash == _sha256(raw_token)
...
class TestValidateToken:
    """Testes da validação pública de tokens de convite."""
    ...
    def _create_token_in_db(self, client_user, db, expires_delta=None):
        ...
        client_user.invite_token_expires_at = utc_now_datetime() + (
...
class TestDefinePasswordAndAutoLogin:
    """Testes de definição de password e auto-login via convite."""
    ...
    def test_set_password_token_invalid_returns_400(self, client, api_key_headers):
