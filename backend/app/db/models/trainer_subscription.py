...
class TrainerSubscription(SQLModel, table=True):
    ...
    # Data de fim do período de trial (15 dias após registo),
    # Usada para bloquear acesso se o trainer não adicionar cartão a tempo
    trial_end: Optional[datetime] = Field(default=None, index=True)
    ...
    # ID da subscrição no Stripe, preenchido quando o
    # trainer adiciona cartão e a subscrição é criada
    stripe_subscription_id: Optional[str] = Field(default=None, index=True)

    # ID do price do Stripe, usado para identificar o plano de pagamento (FREE, STARTER, PRO)
    # e calcular o valor a cobrar
    stripe_price_id: Optional[str] = Field(default=None, index=True)

    # Contagem dos clientes ativos
    # Atualizado sempre que um cliente é adicionado ou arquivado do trainer,
    # para calcular o tier de preço e o valor a cobrar
    active_clients_count: int = Field(default=0, ge=0)

    created_at: datetime = Field(default_factory=utc_now)
    updated_at: datetime = Field(default_factory=utc_now)

    trial_warning_sent: bool = Field(default=False, index=True)
