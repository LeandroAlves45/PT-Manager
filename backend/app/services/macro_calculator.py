...
class MacroGrams:
    ...
    protein_pct: (
        float  # percentagem de calorias provenientes de proteínas (informativo)
    )
    carbs_pct: (
        float  # percentagem de calorias provenientes de carboidratos (informativo)
    )
...
def _harris_benedict(
    weight_kg: float, height_cm: float, age: int, sex: SexType
) -> float:
    """
    Harris-Benedict (revisão Roza & Shizgal, 1984).
...
def calculate_tmb_all_formulas(
    ...
    # Validação para o caso se o sexo for inválido
    if sex not in {"male", "female"}:
        raise ValueError("Sexo inválido. Deve ser 'male' ou 'female'.")

    # Validação para o caso se a chave de atividade for inválida
...
def calculate_macros_from_percentages(
    ...
    Levanta:
        ValueError se as percentagens não somarem aproximadamente 100% ou
        se kcal_target for negativo.
    """
    total_pct = protein_pct + carbs_pct + fats_pct
    if not isclose(total_pct, 100.0, rel_tol=0.0, abs_tol=1.0):
        raise ValueError(
            "As percentagens devem somar aproximadamente 100%. "
            f"Soma atual: {total_pct:.2f}%"
        )
...
def calculate_macros_from_grams_per_kg(
    ...
    # Validação para caso o peso seja menor ou igual a 0
    if weight_kg <= 0:
        raise ValueError("Peso deve ser um valor positivo.")
