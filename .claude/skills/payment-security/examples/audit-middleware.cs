...
public class PaymentAuditMiddleware
{
    ...
    /// Determinar se é operação de pagamento
    private bool IsPaymentOperation(string path)
    {
        return path.Contains("/api/v1/payment-intents", StringComparison.OrdinalIgnoreCase) ||
               path.Contains("/api/v1/payments", StringComparison.OrdinalIgnoreCase) ||
    ...
    private string ExtractEventType(string method, string path)
    {
        ...
        {
            ("POST", var p) when p.Contains("payment-intents") => "payment_intent_created",
            ("GET", var p) when p.Contains("payment-intents") => "payment_intent_retrieved",
            ("POST", var p) when p.Contains("payments") && p.Contains("confirm") => "payment_confirmed",
            ("POST", var p) when p.Contains("refund") => "refund_issued",
            ("GET", var p) when p.Contains("payments") => "payment_retrieved",
