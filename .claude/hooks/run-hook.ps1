...
function Invoke-ProtectFiles {
  ...
  switch -Wildcard ($normalized) {
    ...
    "*/.claude/settings.local.json" { Send-Decision ask "Editing settings.local.json. This controls local permissions and hooks. Confirm this change." }
    "backend/src/infrastructure/migrations/*" { Send-Decision deny "Migration EF Core ja aplicada nao deve ser editada manualmente. Cria uma nova via 'dotnet ef migrations add'." }
    "*/backend/src/infrastructure/migrations/*" { Send-Decision deny "Migration EF Core ja aplicada nao deve ser editada manualmente. Cria uma nova via 'dotnet ef migrations add'." }
...
function Invoke-FormatOnSave {
  ...
  if ($extension -eq ".cs") {
    ...
    if ($dotnet) { & $dotnet.Source format "$projectDir\backend\PTManager.sln" --include "$filePath" *> $null }
...
function Invoke-SessionStart {
  ...
  Write-Output "Stack: C# 14 / .NET 10 / ASP.NET Core / EF Core 10 (backend em reescrita, modular monolith Clean Architecture) + React 19 / Vite / Tailwind"
  ...
  $todoFile = Join-Path $projectDir ".claude\tasks\todo.md"
...
Backend: C# 14, .NET 10, ASP.NET Core, EF Core 10 (modular monolith, Clean Architecture por feature)
...
1. LAYERED ARCHITECTURE - MANDATORY
   Ordem: Api -> Application -> Domain <- Infrastructure, organizado por feature
   Sem IRepository<T> generico, sem MediatR/AutoMapper no MVP
   NUNCA colocar logica de negocio em controllers/endpoints

2. MULTI-TENANT - NON-NEGOTIABLE
   Todas as queries filtram por owner_trainer_id via EF Core Global Query Filters
   ligados a ITenantContext (nunca a HttpContext direto)
   Client role so acede aos proprios dados

3. DATABASE MIGRATIONS - NON-NEGOTIABLE
   Migrations sempre geradas via: dotnet ef migrations add
   NUNCA editar uma migration ja aplicada num ambiente partilhado
   NUNCA escrever migrations a mao

4. ERROR HANDLING
   Result<T> / Problem Details no ASP.NET Core, codigos HTTP corretos
   Nunca expor stack traces ao cliente

5. TESTING REQUIREMENTS
   dotnet test (xUnit) no backend - unit, integration (Testcontainers), functional (WebApplicationFactory)
   Vitest no frontend
   Correr ficheiro/projeto especifico apos alteracoes

6. SECURITY
   Secrets em environment variables
   Queries via EF Core (parametrizadas)
...
COMMANDS:
  dotnet run --project src/Api                        # Start backend (from backend/)
  dotnet ef database update --project src/Infrastructure  # Apply migrations
  dotnet test                                          # Run backend tests
  dotnet format                                         # Format C#
