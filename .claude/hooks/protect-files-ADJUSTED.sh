...
# ADJUSTED FOR PT MANAGER: SQL migration protection
...
case "$PATH_LC" in
  ...
  # ============= ADJUSTED FOR PT MANAGER (FastAPI + React) =============
  */backend/app/db/migrations/*|backend/app/db/migrations/*)
    emit deny "Migrations SQL nao devem ser editadas manualmente. Cria um novo ficheiro numerado em backend/app/db/migrations/" ;;
  docker-compose.yml|docker-compose.*.yml|*/docker-compose.yml|*/docker-compose.*.yml)
    emit ask "Editing docker-compose. Infrastructure changes require review." ;;
  # =================================================
esac
