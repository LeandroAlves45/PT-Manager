...
Backend: Python 3.12, FastAPI, SQLModel, PostgreSQL 16, JWT, Stripe, APScheduler.
Frontend: React 19, Vite 7, Tailwind CSS 4, Chakra UI + shadcn/ui (páginas `.jsx`, UI em `.tsx`).
Deploy: Render (backend) + Vercel (frontend) + Redis(Upstach Redis, TODO) + DB(Render ou Supabase).
...
- Architecture: ver `architecture.md` e `clean-architecture-guide.md`. Camadas em `backend/app/`: `api/routes → services → repositories → db/models`
- Multi-tenant: **todas** as queries filtram por `trainer_id`. Roles: `superuser`, `trainer`, `client`
...
- Base de dados: ver `database-schema.md`. Migrations SQL em `backend/app/db/migrations/` via `python -m app.db.migrate_runner` — nunca editar migration já aplicada; criar novo ficheiro numerado
- Segurança: ver `security-conventions.md`. Secrets em environment variables. Queries via SQLModel/SQLAlchemy (parametrizadas)
- Testing: `pytest` no backend (unit + integration). Correr ficheiro específico após alterações. Vitest no frontend
- Git: feature branches, conventional commits, testes a passar antes de commit
- NO edits: migrations SQL existentes, ficheiros `.env`, valores hardcoded de secrets
...
- Ao documentar código: docstrings Python no backend, JSDoc no frontend. Métodos e classes em inglês
...
- `**/*.pem`, `**/*.key`, `**/secrets/**`
- `**/backend/app/db/migrations/**` — nunca editar migration existente; criar sempre um novo ficheiro SQL numerado
...
pytest
pytest tests/integration/api/test_invites.py -v
ruff check app/
ruff format app/
